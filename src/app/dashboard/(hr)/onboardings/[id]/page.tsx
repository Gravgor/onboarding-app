  ...
  useEffect(() => {
    async function fetchOnboarding() {
      if (session?.user?.companyId) {
        const request = await fetch("/api/hr/get-onboarding", {
          method: "POST",
          headers: {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold">
        Onboarding Details: {onboarding.user.name}
      </h2>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Details about the onboarding employee
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Badge
                variant={onboarding.progress >= 75 ? "default" : "secondary"}
              >
                {onboarding.progress}%
              </Badge>
            </div>
              ...
              (doc: {
                ...
                <TableRow key={doc.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        doc.status === "Signed" ? "default" : "destructive"
                      }
                    >
                      {doc.status}
                    </Badge>
      ...
      <AnimatedCollapsible title="Compliance Progress">
        <div className="space-y-4 p-4">
          {onboarding.complianceItems.map(
            (item: { id: string; name: string; progress: number }) => (
